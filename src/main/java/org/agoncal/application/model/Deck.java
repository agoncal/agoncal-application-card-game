...
public class Deck {
  ...
  public Deck(boolean startShuffled) {
    cards = new Card[53]; // Indices 1-52 (does not use index 0)

    // Fill the deck with cards
    for (int rank = 1; rank <= 13; rank++) {
      // Place cards in order in deck, shuffle later
      cards[rank] = new Card(1, rank); // First suit, ex: 3 of clubs
      cards[rank + 13] = new Card(2, rank); // Second suit, diamonds
      cards[rank + 26] = new Card(3, rank); // Third suit, hearts
      cards[rank + 39] = new Card(4, rank); // Fourth suit, spades
    }
